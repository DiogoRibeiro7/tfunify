...
def optimize_european_tf_grid(prices: np.ndarray, 
                             train_start: int = 0, 
                             train_end: int = None) -> OptimizationResult:
...
def optimize_american_tf_random(prices: np.ndarray,
                               train_start: int = 0,
                               train_end: int = None,
                               n_trials: int = 200) -> OptimizationResult:
...
def optimize_tsmom_bayesian(prices: np.ndarray,
                           train_start: int = 0,
                           train_end: int = None,
...
def optimize_tsmom_random(prices: np.ndarray,
                         train_start: int = 0,
                         train_end: int = None,
...
def walk_forward_analysis(prices: np.ndarray, 
                         system_configs: Dict[str, Any],
                         train_window: int = 1000,
                         test_window: int = 250,
                         reoptimize_freq: int = 250) -> Dict[str, Dict]:
    """
...
def create_optimization_plots(optimization_results: List[OptimizationResult], 
                             walkforward_results: Dict[str, Dict]) -> None:
...
def print_optimization_summary(optimization_results: List[OptimizationResult]) -> None:
    ...
    for result in optimization_results:
        method = "Grid Search" if "European" in result.system_name else \
                "Random Search" if "American" in result.system_name else \
        ...
        print(f"{result.system_name:<15} {method:<20} "
    ...
    print(f"- Best performing system: {best_result.system_name} (score: {best_result.best_score:.3f})")
    ...
    for result in optimization_results:
        if result.best_params and result.system_name == "European TF":
            ...
            span_ratio = result.best_params.get('span_long', 1) / result.best_params.get('span_short', 1)
...
def main():
    ...
    default_configs = {
        'European TF': optimization_results[0].best_params or {
            'sigma_target_annual': 0.12, 'a': 252, 'span_sigma': 44,
            'mode': 'longshort', 'span_long': 200, 'span_short': 40
        },
        'American TF': optimization_results[1].best_params or {
            'span_long': 150, 'span_short': 30, 'atr_period': 30,
            'q': 2.5, 'p': 4.0, 'r_multiple': 0.015
        },
        'TSMOM': optimization_results[2].best_params or {
            'sigma_target_annual': 0.10, 'a': 252, 'span_sigma': 44,
            'L': 15, 'M': 8
        }
    }
