...
def calculate_detailed_metrics(pnl: np.ndarray, name: str) -> PerformanceMetrics:
    ...
    if len(valid_pnl) == 0:
        return PerformanceMetrics(
            name=name, total_return=0, annual_return=0, annual_volatility=0,
            sharpe_ratio=0, max_drawdown=0, calmar_ratio=0, win_rate=0,
            profit_factor=0, var_95=0, skewness=0, kurtosis=0, valid_days=0
        )
...
def run_scenario_comparison(scenarios: Dict[str, np.ndarray], 
                          configs: Dict[str, Tuple]) -> Dict[str, Dict[str, PerformanceMetrics]]:
    ...
    for scenario_name, prices in scenarios.items():
        ...
        for system_name, (system_class, config) in configs.items():
            try:
                ...
                print(f"  {system_name:<12}: Sharpe={metrics.sharpe_ratio:5.2f}, "
                      f"Return={metrics.annual_return:6.2%}, "
                      f"MaxDD={metrics.max_drawdown:6.2f}")
                
            except Exception as e:
                print(f"  {system_name:<12}: Error - {e}")
                scenario_results[system_name] = calculate_detailed_metrics(np.array([]), system_name)
...
def create_comparison_plots(results: Dict[str, Dict[str, PerformanceMetrics]]) -> None:
    ...
    metrics_data = {
        'sharpe_ratio': np.array([[results[scenario][system].sharpe_ratio 
                                  for system in systems] for scenario in scenarios]),
        'annual_return': np.array([[results[scenario][system].annual_return 
                                   for system in systems] for scenario in scenarios]),
        'max_drawdown': np.array([[results[scenario][system].max_drawdown 
                                  for system in systems] for scenario in scenarios]),
        'win_rate': np.array([[results[scenario][system].win_rate 
                              for system in systems] for scenario in scenarios])
    }
    ...
    for i in range(len(scenarios)):
        for j in range(len(systems)):
            text = ax1.text(j, i, f'{metrics_data["sharpe_ratio"][i, j]:.2f}',
                           ha="center", va="center", color="black" if abs(metrics_data["sharpe_ratio"][i, j]) < 1 else "white")
    ...
    for i, system in enumerate(systems):
        profit_factors = [min(results[scenario][system].profit_factor, 5) for scenario in scenarios]  # Cap at 5 for visualization
    ...
    for system in systems:
        ...
        for scenario in scenarios:
            ...
            score = (m.sharpe_ratio * 0.4 + 
                    ...
                    m.win_rate * 0.2 + 
                    min(m.profit_factor, 5) * 0.1)   # Cap profit factor at 5
            scores.append(score)
...
def run_monte_carlo_analysis(configs: Dict[str, Tuple], n_simulations: int = 100) -> None:
    ...
    print(f"{'System':<15} {'Mean Sharpe':<12} {'Std Sharpe':<12} {'% Positive':<12} {'95% CI':<15}")
    print("-" * 75)
    ...
    for system_name, sharpes in mc_results.items():
        if len(sharpes) > 0:
            ...
            print(f"{system_name:<15} {mean_sharpe:<12.2f} {std_sharpe:<12.2f} "
...
def main():
    ...
    for name, prices in scenarios.items():
        ...
        print(f"  {name}: {len(prices)} days, "
              f"Return={annual_ret:.2%}, Vol={annual_vol:.1%}")
    ...
    for scenario_name, scenario_results in results.items():
        ...
        print("-" * 60)
        print(f"{'System':<15} {'Sharpe':<8} {'Return':<8} {'Vol':<8} {'MaxDD':<8} {'WinRate':<8} {'PF':<6}")
        print("-" * 60)
        
        for system_name, metrics in scenario_results.items():
            if metrics.valid_days > 0:
                print(f"{system_name:<15} "
    ...
    sorted_systems = sorted(system_averages.items(), 
                           key=lambda x: x[1]['sharpe'], reverse=True)
    
    print(f"{'Rank':<5} {'System':<15} {'Avg Sharpe':<12} {'Avg Return':<12} {'Avg Vol':<10} {'Avg MaxDD':<10}")
    print("-" * 70)
    
    for rank, (system, metrics) in enumerate(sorted_systems, 1):
        print(f"{rank:<5} {system:<15} "
